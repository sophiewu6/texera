...
namespace {
	...
	using miao_atom = atom_constant<atom("miao")>;
	...
	behavior calculator_fun(event_based_actor* self) {
		return {
			...
			[=](miao_atom,actor_addr ac)
		{
			auto hdl = actor_cast<actor>(ac);
			aout(self) << hdl->address() << endl;
			self->send(hdl, miao_atom::value, "miao");
		}	
		};
	}
	...
	//                    +-------------+
		...
		void connecting(stateful_actor<state>* self,
			...
			self->request(mm, infinite, connect_atom::value, host, port).await(
				...
				self->monitor(hdl);
				self->send(hdl, miao_atom::value, self->address());
